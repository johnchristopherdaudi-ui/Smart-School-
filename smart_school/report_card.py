...
def get_report_card_data(term_result):
	...
	exams = frappe.get_all(
		"Exam",
		filters={"term": doc.term, "class": doc.get("class")},
		fields=["name", "exam_name", "max_marks", "weight"],
	...
	return frappe._dict(
		...
		position=position,
		out_of=len(ranked),
		position_text=f"Position {position} out of {len(ranked)}"
		if position
...
def download_report_card(student, term):
	...
	result = frappe.db.get_value(
		"Student Term Result", {"student": student, "term": term}, ["name", "class"], as_dict=True
	)
	if not result or not is_term_summary_published(term, result["class"]):
		frappe.throw(
			"Report card itapatikana matokeo yote ya muhula huu yakishachapishwa", frappe.PermissionError
		)
...
def get_class_report_cards(class_name, term):
	...
	names = dict(
		frappe.get_all(
			"Student Term Result",
			filters={"class": class_name, "term": term},
			fields=["student", "name"],
			as_list=True,
		)
	)
