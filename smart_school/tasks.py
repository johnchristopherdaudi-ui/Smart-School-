...
def get_current_term():
    ...
    term = frappe.get_all(
        "Term", filters={"start_date": ["<=", today()]}, fields=["name", "start_date", "end_date"],
...
def get_previous_term(term):
    previous = frappe.get_all(
        "Term", filters={"start_date": ["<", term.start_date]}, pluck="name", order_by="start_date desc", limit=1
    )
...
def calculate_risk_score(student, term=None, settings=None):
    ...
    parts["attendance"] = {"days": len(statuses), "absent": statuses.count("Absent"), "late": statuses.count("Late"),
                           "excused": statuses.count("Excused"), "rate": flt(absence_rate, 1),
    ...
    parts["discipline"] = {"records": len(severities), "points": points, "part": min(points / FULL_DISCIPLINE_POINTS, 1)}
    ...
    if average is None:
        ...
        parts["low_average"] = {"average": flt(average, 2), "threshold": threshold,
        ...
        parts["decline"] = {"previous_term": previous_term, "previous_average": previous_average,
                            "drop": flt(drop, 2), "part": min(drop / FULL_DECLINE, 1)}
...
def check_class_performance(class_name):
    ...
    for subject in sorted(set(current["subjects"]) & set(previous["subjects"])):
        save_insight(class_name, current_term, subject,
                     mean(current["subjects"][subject]), mean(previous["subjects"][subject]))


def get_class_term_scores(class_name, term):
    results = frappe.get_all("Student Term Result", filters={"class": class_name, "term": term},
...
def save_insight(class_name, term, subject, current_avg, previous_avg):
    ...
    existing = frappe.db.get_value(
        "Performance Insight", {"class": class_name, "term": term, "subject": subject or ["is", "not set"]}, "name"
    )
    ...
    message = f"{what} wa {class_name} {direction} kwa {abs(flt(change, 1))}% ukilinganisha na muhula uliopita."

    doc = frappe.get_doc("Performance Insight", existing) if existing else frappe.new_doc("Performance Insight")
    doc.update({
        "class": class_name,
        "term": term,
        "subject": subject,
...
def create_academic_records_for_ended_years():
    ...
    for year in frappe.get_all("Academic Year", filters={"end_date": ["<=", today()]}, pluck="name"):
        for student in frappe.get_all("Student", filters={"status": ["in", ["Active", "Graduated"]]}, pluck="name"):
...
def ensure_academic_record(student, academic_year, student_class=None):
    ...
    record.set("class", last.get("class") or student_class or frappe.db.get_value("Student", student, "current_class"))
