...
def validate_non_overlapping_range(doc, low_field, high_field, unique_fields=()):
    ...
    if low > high:
        frappe.throw(f"{doc.meta.get_label(low_field)} cannot be greater than {doc.meta.get_label(high_field)}")
    ...
    for fieldname in unique_fields:
        other = frappe.db.get_value(doc.doctype, {fieldname: doc.get(fieldname), "name": ["!=", doc.name]}, "name")
        if other:
...
def get_subject_scores(student, term):
    ...
    exams = {
        e.name: e
        for e in frappe.get_all("Exam", filters={"term": term}, fields=["name", "class", "weight"])
    }
...
def recompute_student_term_result(student, term):
    ...
    doc = frappe.get_doc("Student Term Result", existing) if existing else frappe.new_doc("Student Term Result")
    doc.update({
        "student": student,
        "term": term,
        "class": exam_class,
...
def recompute_term_results(exam):
    ...
    students = frappe.get_all("Exam Result", filters={"exam": exam, "docstatus": 1}, pluck="student", distinct=True)
...
def notify_published_exam(exam):
    ...
    for student, rows in by_student.items():
        ...
        marks = ", ".join(f"{r.subject} {flt(r.marks):g}/{flt(exam_doc.max_marks):g} ({r.grade})" for r in rows)
...
def get_portal_results(student):
    ...
    exams = {
        ...
        for e in frappe.get_all(
            "Exam", filters={"results_published": 1}, fields=["name", "exam_name", "term", "class", "max_marks"]
        )
    }
    rows = frappe.get_all(
        "Exam Result",
    ...
    summaries = {
        ...
        for s in frappe.get_all(
            ...
            fields=["term", "class", "average", "division", "division_display", "total_points", "subjects_count"],
        )
    }
    ...
    for r in rows:
        ...
        term.subjects.append(frappe._dict(
            exam_name=exam.exam_name, subject=r.subject, marks=flt(r.marks), max_marks=flt(exam.max_marks), grade=r.grade
        ))
