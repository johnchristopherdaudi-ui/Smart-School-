...
SEVERITY_KEYWORDS = [("Minor", r"low|minor|ndogo"), ("Serious", r"high|serious|severe|kubwa"), ("Moderate", r"medium|moderate|wastani")]
    ...
    for r in frappe.get_all("Discipline Record", fields=["name", "incident_type", "severity", "description"]):
        ...
        if severity not in ("Minor", "Moderate", "Serious"):
            values["severity"] = next(
                (option for option, pattern in SEVERITY_KEYWORDS if re.search(pattern, severity, re.I)), "Moderate"
            )
