...
def execute():
    ...
    frappe.db.set_value("Payment Gateway Log", {"provider": "M-pesa"}, "provider", "M-Pesa", update_modified=False)

    logs = frappe.get_all(
        "Payment Gateway Log", filters={"term": ["is", "not set"], "fee_payment": ["is", "set"]}, fields=["name", "fee_payment"]
    )
    for log in logs:
