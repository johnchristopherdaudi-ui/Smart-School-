...
def execute():
    ...
    for guardian in frappe.get_all("Guardian", filters={"email": ["is", "set"]}, fields=["name", "email", "user"]):
    ...
    parent_users = frappe.get_all("Has Role", filters={"role": "Parent", "parenttype": "User"}, pluck="parent")
