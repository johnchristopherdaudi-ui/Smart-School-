...
def get_dashboard(student):
    ...
    if data.latest:
        ...
        if failed:
            data.alerts.append(frappe._dict(kind="results", text=(
    ...
    statuses = frappe.get_all("Attendance", filters={"student": student.name, "term": term.name}, pluck="status") if term else []
    ...
    if data.attendance.rate is not None and len(statuses) >= MIN_DAYS_FOR_ATTENDANCE_ALERT \
            and data.attendance.rate < ATTENDANCE_ALERT_BELOW:
        data.alerts.append(frappe._dict(kind="attendance", text=(
    ...
    if term and frappe.db.count(
        ...
        data.alerts.append(frappe._dict(kind="discipline", text=(
    ...
    if statement.balance > 0:
        data.alerts.append(frappe._dict(kind="fees", text=f"Kuna deni la ada la {statement.balance:,.0f} TZS."))

    # Announcements of the last 30 days
    recent = get_announcements([student.current_class] if student.current_class else [], since=add_days(nowdate(), -30))
    data.announcements = frappe._dict(count=len(recent), latest=recent[0].title if recent else None)

    if not data.alerts:
        data.alerts.append(frappe._dict(kind="ok", text=f"Hakuna jambo la kuhofia kwa sasa. Endelea kumtia moyo {first_name}!"))
    return data
