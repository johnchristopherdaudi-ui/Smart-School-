...
def get_teacher_scope():
    ...
    if teacher:
        for row in frappe.get_all(
            "Teacher Subject Assignment", filters={"parenttype": "Teacher", "parent": teacher}, fields=["class", "subject"]
        ):
...
def get_division_distribution():
    ...
    return {"labels": list(counts), "datasets": [{"name": f"Divisions ({term})", "values": list(counts.values())}]}
...
def get_fee_collection_by_term():
    ...
    return {"labels": [r.term for r in rows], "datasets": [{"name": "Collected", "values": [flt(r.total) for r in rows]}]}
