...
from smart_school.tests.factory import SUBJECTS, TEACHER_1, SchoolTestCase, add_result, as_user, make_exam, no_commit
...
class TestRiskScore(SchoolTestCase):
    def test_parts_weights_and_level(self):
        ...
        for subject in SUBJECTS[:7]:
            ...
            add_result(student, current, subject, {"_T MATH": 20, "_T ENGLISH": 25}.get(subject, 40))  # avg 35, two F
        start = frappe.db.get_value("Term", "_Test T3", "start_date")
        for i, status in enumerate(["Absent", "Absent", "Late", "Late", "Excused"] + ["Present"] * 5):
            frappe.get_doc({"doctype": "Attendance", "student": student, "date": add_days(start, i), "status": status}).insert(
                ignore_permissions=True)
        for severity in ("Minor", "Serious"):
            frappe.get_doc({"doctype": "Discipline Record", "student": student, "date": today(), "incident_type": "Lateness",
                            "severity": severity}).insert(ignore_permissions=True)
        ...
        self.assertEqual({k: v["score"] for k, v in breakdown.items() if isinstance(v, dict)},
                         {"attendance": 25.0, "discipline": 12.5, "low_average": 5.6, "failed_subjects": 10.0, "decline": 15.0})
    ...
    def test_levels_and_settings_validation(self):
        ...
        self.assertEqual([tasks.get_risk_level(x, settings) for x in (29.9, 30, 59.9, 60)], ["Low", "Medium", "Medium", "High"])
...
class TestInsightsRecordsAndPromotion(SchoolTestCase):
    def test_insights_created_then_updated(self):
        ...
        insight = frappe.db.get_value("Performance Insight", {"class": "_Test FORM 1", "term": "_Test T2", "subject": ["is", "not set"]},
                                      ["name", "change_percentage"], as_dict=True)
        self.assertEqual(insight.change_percentage, -33.3)
        self.assertTrue(frappe.db.exists("Performance Insight", {"class": "_Test FORM 1", "term": "_Test T2", "subject": "_T MATH"}))

        for subject in SUBJECTS[:7]:
            add_result(self.s["b"], t1, subject, 60)
            add_result(self.s["b"], t2, subject, 60)
        tasks.check_class_performance("_Test FORM 1")
        updated = frappe.db.get_value("Performance Insight", {"class": "_Test FORM 1", "term": "_Test T2", "subject": ["is", "not set"]},
                                      ["name", "change_percentage"], as_dict=True)
    ...
    def test_promotion_tool(self):
        ...
        self.assertEqual(frappe.db.get_value("Student Academic Record", {"student": self.s["a"], "academic_year": "_Test AY"}, "class"),
                         "_Test FORM 1")
    ...
    def test_top_class_graduates(self):
        student = frappe.get_doc({"doctype": "Student", "full_name": "_Test Leaver", "current_class": "_Test FORM 4",
    ...
    def test_academic_records_for_ended_years(self):
        frappe.get_doc({"doctype": "Academic Year", "year": "_Test Old AY", "start_date": add_days(today(), -700),
                        "end_date": add_days(today(), -400)}).insert(ignore_permissions=True, set_name="_Test Old AY")
        frappe.get_doc({"doctype": "Term", "term_name": "Term 3", "academic_year": "_Test Old AY", "start_date": add_days(today(), -500),
                        "end_date": add_days(today(), -401)}).insert(ignore_permissions=True, set_name="_Test Old T3")
        frappe.get_doc({"doctype": "Student Term Result", "student": self.s["e"], "term": "_Test Old T3", "class": "_Test FORM 1",
                        "average": 51, "division": "Division III"}).insert(ignore_permissions=True)
        with no_commit():  # the daily job commits
            tasks.create_academic_records_for_ended_years()
        self.assertEqual(frappe.db.get_value("Student Academic Record", {"student": self.s["e"], "academic_year": "_Test Old AY"}, "class"),
                         "_Test FORM 1")
