...
SUBJECTS = ("_T MATH", "_T ENGLISH", "_T KISWAHILI", "_T PHYSICS", "_T BIOLOGY", "_T HISTORY", "_T GEOGRAPHY", "_T CIVICS")
...
def make_school():
    ...
    get_or_insert("Academic Year", YEAR, {"year": YEAR, "start_date": start, "end_date": add_days(today(), 160)})
    for name, (begin, end) in zip(TERMS, [(-180, -100), (-90, -10), (-5, 80)]):
        get_or_insert("Term", name, {"term_name": f"Term {name[-1]}", "academic_year": YEAR,
                                     "start_date": add_days(today(), begin), "end_date": add_days(today(), end)})
    ...
    for class_name in ("_Test FORM 1", "_Test FORM 2"):
        for subject in SUBJECTS:
            if not frappe.db.exists("Class Subject Mapping", {"class": class_name, "subject": subject}):
                frappe.get_doc({"doctype": "Class Subject Mapping", "class": class_name, "subject": subject,
                                "subject_scope": "All Combinations"}).insert(ignore_permissions=True)
    for class_name in ("_Test FORM 1", "_Test FORM 2"):
        for term, amount in FEES.items():
            if not frappe.db.exists("Fee Structure", {"class": class_name, "term": term}):
                frappe.get_doc({"doctype": "Fee Structure", "class": class_name, "term": term, "amount": amount}).insert(
                    ignore_permissions=True)

    teacher_1 = make_teacher(TEACHER_1, "_Test Teacher One", [("_T MATH", "_Test FORM 1"), ("_T ENGLISH", "_Test FORM 1")])
...
def make_teacher(email, full_name, assignments):
    ...
    if not name:
        doc = frappe.get_doc({"doctype": "Teacher", "full_name": full_name, "email": email,
...
def make_student(full_name, class_name, gender):
    ...
    if not name:
        name = frappe.get_doc({"doctype": "Student", "full_name": full_name, "current_class": class_name,
                               "gender": gender, "status": "Active"}).insert(ignore_permissions=True).name
    return name
...
def make_guardian(full_name, email, phone, students):
    ...
    if not name:
        doc = frappe.get_doc({"doctype": "Guardian", "full_name": full_name, "email": email,
                              "phone": phone,
...
def make_exam(exam_name, term, class_name, max_marks=100, weight=None, published=0):
    ...
    if not name:
        name = frappe.get_doc({"doctype": "Exam", "exam_name": exam_name, "term": term, "class": class_name,
                               "max_marks": max_marks, "weight": weight}).insert(ignore_permissions=True).name
    frappe.db.set_value("Exam", name, "results_published", published)
    return name


def add_result(student, exam, subject, marks):
    doc = frappe.get_doc({"doctype": "Exam Result", "student": student, "exam": exam, "subject": subject, "marks": marks})
    doc.insert(ignore_permissions=True)
    doc.submit()
    return doc
...
def pay(student, term, amount):
    doc = frappe.get_doc({"doctype": "Fee Payment", "student": student, "term": term, "amount_paid": amount,
                          "payment_date": today(), "payment_method": "Cash", "status": "Partial"})
    doc.insert(ignore_permissions=True)
    doc.submit()
    return doc
