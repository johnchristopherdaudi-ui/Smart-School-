...
class TestResultEngine(SchoolTestCase):
    ...
    def term_result(self, student):
        return frappe.db.get_value("Student Term Result", {"student": student, "term": "_Test T3"},
                                   ["average", "subjects_count", "total_points", "division"], as_dict=True)
    ...
    def test_best_seven_division_and_incomplete(self):
        ...
        self.assertEqual((result.subjects_count, result.total_points, result.division), (7, 21, "Division II"))
    ...
    def test_amend_after_cancel(self):
        ...
        original.cancel()
        self.assertFalse(frappe.db.exists("Exam Result", {"student": self.s["b"], "subject": "_T GEOGRAPHY", "docstatus": 1}))
...
class TestGradingAndExamRules(SchoolTestCase):
    def test_overlapping_ranges_rejected(self):
        grade = frappe.get_doc({"doctype": "Grading System", "grade": "A", "minimum_mark": 70, "maximum_mark": 80, "points": 9})
        self.assertRaises(frappe.ValidationError, grade.insert, set_name="_Test Grade")
        division = frappe.get_doc({"doctype": "Division Grading", "division": "_Test", "minimum_points": 30, "maximum_points": 40})
    ...
    def test_weights_all_or_none_and_at_most_100(self):
        ...
        unweighted = frappe.get_doc({"doctype": "Exam", "exam_name": "_Test W2", "term": "_Test T2", "class": "_Test FORM 2"})
        self.assertRaises(frappe.ValidationError, unweighted.insert)
        too_much = frappe.get_doc({"doctype": "Exam", "exam_name": "_Test W3", "term": "_Test T2", "class": "_Test FORM 2", "weight": 70})
        self.assertRaises(frappe.ValidationError, too_much.insert)

    def test_term_dates_validated(self):
        term = frappe.get_doc({"doctype": "Term", "term_name": "Term 1", "academic_year": "_Test AY",
