...
from smart_school.tests.factory import (
    PARENT_1, PARENT_2, TEACHER_1, TEACHER_2, SchoolTestCase, add_result, as_user, call, make_exam,
)
...
class TestGuestAndParentPermissions(SchoolTestCase):
    def test_guest_cannot_read_or_approve_admissions(self):
        ...
        admission = frappe.get_doc({"doctype": "Student Admission", "full_name": "_Test Applicant", "date_of_birth": "2012-01-01",
                                    "parent_name": "_Test Parent", "phone_number": "+255 754 000 303",
                                    "class_applying": "_Test FORM 1"}).insert(ignore_permissions=True)
        with as_user("Guest"):
    ...
    def test_approval_needs_a_certificate_or_verification(self):
        frappe.set_user("Administrator")
        doc = frappe.get_doc({"doctype": "Student Admission", "full_name": "_Test Applicant Two", "date_of_birth": "2012-01-01",
                              "parent_name": "_Test Parent", "phone_number": "+255 754 000 101",
                              "class_applying": "_Test FORM 1"}).insert(ignore_permissions=True)
        doc.status = "Approved"
    ...
    def test_parent_has_no_doctype_access(self):
        with as_user(PARENT_1):
            for doctype in ("Guardian", "Fee Payment", "Attendance", "Student", "Student Term Result", "Exam Result",
                            "Announcement", "Fee Structure", "Class", "Subject"):
    ...
    def test_student_photo_only_for_own_children(self):
        with as_user(PARENT_2):
            self.assertRaises(frappe.PermissionError, call, "smart_school.portal_utils.get_student_photo", student=self.s["a"])
...
class TestTeacherPermissions(SchoolTestCase):
    def test_teacher_sees_only_own_exam_results(self):
        ...
        with as_user(TEACHER_1):
            teachers = {r.teacher for r in frappe.get_list("Exam Result", fields=["teacher"], limit_page_length=0)}
        self.assertEqual(teachers, {self.school.teacher_1})

    def test_teacher_cannot_set_teacher_field(self):
        exam = make_exam("_Test Perm Exam", "_Test T2", "_Test FORM 1")
        with as_user(TEACHER_1):
            doc = frappe.get_doc({"doctype": "Exam Result", "student": self.s["b"], "exam": exam, "subject": "_T ENGLISH",
                                  "marks": 40, "teacher": self.school.teacher_2}).insert()
    ...
    def test_comments_by_their_owners_only(self):
        ...
        name = frappe.db.get_value("Student Term Result", {"student": self.s["e"], "term": "_Test T2"}, "name")
        ...
        self.assertRaises(frappe.PermissionError, write, TEACHER_2, "headmaster_comment", "Not the headmaster")
