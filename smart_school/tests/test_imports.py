...
from smart_school.tests.factory import (
    PARENT_1, TEACHER_1, SchoolTestCase, add_result, as_user, call, make_exam, make_student,
)
...
def make_file(name, text):
    return frappe.get_doc({"doctype": "File", "file_name": name, "is_private": 1, "content": text.encode("utf-8")}).insert(
        ignore_permissions=True).file_url
...
class TestMarksImport(SchoolTestCase):
    ...
    def test_parent_refused(self):
        with as_user(PARENT_1):
            self.assertRaises(frappe.PermissionError, call, API + "bulk_create_exam_results", exam=self.exam,
                              subject="_T MATH", entries=[{"student": self.s["a"], "marks": 40}])

    def test_unassigned_teacher_refused(self):
        with as_user(TEACHER_1):  # teaches MATH and ENGLISH only
            self.assertRaises(frappe.PermissionError, call, API + "bulk_create_exam_results", exam=self.exam,
                              subject="_T PHYSICS", entries=[{"student": self.s["a"], "marks": 40}])

    def test_csv_with_bom_ids_names_and_row_errors(self):
        with as_user(TEACHER_1):
            url = make_file("_test_marks.csv", "﻿Admission Number,Marks\n"
                            f"{self.s['a']},45\n_Test Student B,30\nNobody Here,20\n{self.s['c']},abc\n{self.s['d']},60\n")
        ...
        teacher = frappe.db.get_value("Exam Result", {"exam": self.exam, "student": self.s["a"], "subject": "_T ENGLISH"}, "teacher")
    ...
    def test_wide_csv_by_code_and_name(self):
        ...
        with as_user(TEACHER_1):
            url = make_file("_test_wide.csv", f"Student,{code},_t english,_T PHYSICS,NOSUCH\n{self.s['c']},40,35,20,10\n")
    ...
    def test_ambiguous_name_needs_admission_number(self):
        ...
        frappe.get_doc({"doctype": "Student", "full_name": "_Test Twin", "current_class": "_Test FORM 1",
                        "status": "Active"}).insert(ignore_permissions=True)
        with as_user(TEACHER_1):
            summary = call(API + "bulk_create_exam_results", exam=self.exam, subject="_T MATH",
