...
from smart_school.tests.factory import (
    ACCOUNTANT, HEADMASTER, SUBJECTS, TEACHER_1, TEACHER_2, SchoolTestCase, add_result, as_user, make_exam, pay,
)
...
class TestReports(SchoolTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.incomplete = frappe.get_doc({"doctype": "Student", "full_name": "_Test Six Subjects", "current_class": "_Test FORM 1",
    ...
    def test_teacher_limited_to_assigned_classes_and_subjects(self):
        with as_user(TEACHER_1):
            subjects = {r["subject"] for r in rows("Subject Performance", term="_Test T2", **{"class": "_Test FORM 1"})}
            self.assertEqual(subjects, {"_T MATH", "_T ENGLISH"})
            self.assertRaises(frappe.PermissionError, rows, "Class Merit List", **{"class": "_Test FORM 2", "term": "_Test T2"})
    ...
    def test_accountant_limited_to_fee_reports(self):
        with as_user(ACCOUNTANT):
            self.assertRaises(frappe.PermissionError, rows, "Class Merit List", **{"class": "_Test FORM 1", "term": "_Test T2"})
    ...
    def test_pending_comments(self):
        with as_user(TEACHER_2):  # class teacher of FORM 2 only
            self.assertEqual(rows("Pending Report Card Comments", term="_Test T2", comment="Class Teacher"), [])
            self.assertRaises(frappe.PermissionError, rows, "Pending Report Card Comments", term="_Test T2", comment="Headmaster")
        with as_user(HEADMASTER):
