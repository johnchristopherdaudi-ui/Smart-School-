...
from smart_school.tests.factory import (
    ACCOUNTANT, PARENT_1, PARENT_2, SchoolTestCase, as_user, call, no_commit, pay, render,
)
...
class TestPaymentRequests(SchoolTestCase):
    ...
    def test_request_validation(self):
        for bad in [dict(amount="0"), dict(amount="99999999"), dict(amount="abc"), dict(phone_number="12345"),
                    dict(provider="M-pesa"), dict(term='_Test T3";alert(1);//'), dict(student=self.s["e"])]:
    ...
    def test_mpesa_partial_payment_stored_with_normalised_phone(self):
        ...
        log = frappe.db.get_value("Payment Gateway Log", {"transaction_reference": ref},
                                  ["term", "amount", "phone_number", "provider", "status"], as_dict=True)
        self.assertEqual((log.term, log.amount, log.phone_number, log.provider, log.status),
    ...
    def test_second_pending_request_cannot_overpay(self):
        ...
        self.assertEqual(frappe.db.get_value("Payment Gateway Log", {"transaction_reference": second}, "status"), "Failed")
...
class TestFeeStatementAndPayments(SchoolTestCase):
    ...
    def test_draft_not_counted_and_one_notification_on_submit(self):
        ...
        with patch(f"{FEE_PAYMENT_MODULE}.send_notification") as notify, as_user(ACCOUNTANT):
            doc = frappe.get_doc({"doctype": "Fee Payment", "student": self.s["a"], "term": "_Test T2", "amount_paid": 50000,
                                  "payment_date": frappe.utils.today(), "payment_method": "Cash", "status": "Partial"}).insert()
    ...
    def test_zero_amount_and_parent_refused(self):
        ...
        with as_user(PARENT_1):
            doc = frappe.get_doc({"doctype": "Fee Payment", "student": self.s["a"], "term": "_Test T1", "amount_paid": 1,
                                  "payment_date": frappe.utils.today(), "payment_method": "Cash", "status": "Partial"})
