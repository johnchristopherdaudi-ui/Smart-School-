...
from smart_school.tests.factory import (
    HEADMASTER, PARENT_1, PARENT_2, SUBJECTS, TEACHER_1, SchoolTestCase, add_result, as_user, call, make_exam, render,
)
...
class TestReportCard(SchoolTestCase):
    ...
    def setUpClass(cls):
        ...
        frappe.get_doc({"doctype": "Attendance", "student": cls.s["a"], "status": "Absent",
                        "date": frappe.db.get_value("Term", "_Test T2", "start_date")}).insert(ignore_permissions=True)
        cls.result = frappe.db.get_value("Student Term Result", {"student": cls.s["a"], "term": "_Test T2"}, "name")
    ...
    def test_card_data(self):
        ...
        self.assertEqual((math.exam_marks, math.score, math.grade, math.remark, math.points), ([40, 80], 80, "A", "Excellent", 1))
    ...
    def test_class_printing_headmaster_only(self):
        ...
        with as_user(TEACHER_1):
            self.assertRaises(frappe.PermissionError, report_card.get_class_report_cards, "_Test FORM 1", "_Test T2")
